...
export class cliElectronInquiry {
      ...
      async inquiry() {
            ...
            const projectDir = this.main.meta.projectDir;
            const ucConfig = this.main.ucConfig;
            const x = extractPathConfig(ucConfig);
...
Main file
==> `, `${join(x.srcDec.dirPath, 'main/index.ts')}`));


            const preloadFilePath = join(projectDir, await ask(`Preload file
==>`, `${join(x.srcDec.dirPath, 'preload/index' + options.fileExt)}`));
            const rendererIndexFilePath = ucConfig.browser.baseCodePath ?? join(projectDir, await ask(`Renderer Ts file (load with HTML file)
==>`, `${join(x.srcDec.dirPath, 'renderer/index' + options.fileExt)}`));
            const rendererHtmlFilePath = ucConfig.browser.baseHtmlPath ?? join(projectDir, (ucConfig.browser.baseHtmlPath ?? '') ?? await ask(`Renderer HTML file
==>`, `${join(x.srcDec.dirPath, 'renderer/index.html')}`));
...
==>`, options.removeMenu);
            ...
            const resourceFilePath = join(projectDir, x.srcDec.dirPath, ucConfig.preference.build.ResourceStorageFile);
